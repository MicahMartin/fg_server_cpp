...
inline ExecutionCode VirtualMachine::run() {
  const char *unreachable =
      R""""(If you're seeing this, the code is in what I thought was an unreachable state. 
...
#define READ_SHORT()                                                           \
  (instructionPointer += 2,                                                    \
   (uint16_t)((instructionPointer[-2] << 8) | instructionPointer[-1]))
...
#define BINARY_OP(valueType, op)                                               \
  ...
  for (;;) {
    if (debugMode) {
      ...
      // }
      printf("\n");
      scriptPointer->disassembleInstruction(
          (int)(instructionPointer - scriptPointer->scriptStart()));
    }
    ...
    case OP_GET_ENTITY_STATUS: {
      ...
      godot::UtilityFunctions::print("checked entity status for ", (int)operand,
                                     " got ", (int)val);
...
inline bool VirtualMachine::valuesEqual(Value valueA, Value valueB) {
  ...
  case VAL_STRING:
    return *(AS_STRING(valueA)) == *(AS_STRING(valueB));
  }
}
