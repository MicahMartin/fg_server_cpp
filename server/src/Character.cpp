...
void Character::init(const char *path) {
  ...
  //   godot::UtilityFunctions::print("SPECIAL STATE MAP: ", i.first, "|" ,
  //   i.second);
  // }
...
void Character::loadStates(const char *path) {
  ...
  for (auto i : stateJson.at("entities").items()) {
    if (i.value().count("defPath")) {
      ...
      auto &newEntity = entityList.emplace_back(this, i.value().at("entityID"),
                                                defString.c_str());
      newEntity.init();
  ...
  for (auto i : stateJson.at("animation_assets").items()) {
    ...
    // visualEffects.emplace(visualID,
    // VisualEffect{}).first->second.anim.loadAnimEvents(i.value().at("animation"));
  ...
  for (auto i : stateJson.at("hurt_sounds").items()) {
    ...
    // hurtSoundEffects.emplace(hurtSoundID, SoundObj{hurtSoundPointer, false,
    // 0, soundChannel + 1});
  }
  // loadSpecialStates
  {
    ...
    specialStateMap[SS_FORWARD_THROW_ATTEMPT] =
        i.at("SS_FORWARD_THROW_ATTEMPT");
    specialStateMap[SS_FORWARD_THROW_SUCCESS] =
        i.at("SS_FORWARD_THROW_SUCCESS");
...
void Character::update() {
  ...
  for (auto i : currentState->visualEffectMap) {
    ...
    // printf("found visFX for frame %d, the playLEngth %d\n",
    // currentState->stateTime, visFX.getPlayLength()); if (visFX.getAura()) {
...
void Character::updateCollisionBoxPositions() {
  for (auto &cbId : currentState->pushBoxIds) {
    getCollisionBox(cbId).positionX =
        position.first - (getCollisionBox(cbId).width / 2);
    getCollisionBox(cbId).positionY =
        position.second + getCollisionBox(cbId).offsetY;
  }

  for (auto &cbId : currentState->hurtBoxIds) {
    getCollisionBox(cbId).positionX =
        position.first + (faceRight ? getCollisionBox(cbId).offsetX
                                    : -(getCollisionBox(cbId).offsetX +
                                        getCollisionBox(cbId).width));
    getCollisionBox(cbId).positionY =
        position.second + getCollisionBox(cbId).offsetY;
  }
...
void Character::updateCollisionBoxes() {
  ...
  for (auto &cbId : currentState->pushBoxIds) {
    ...
    if ((getCollisionBox(cbId).end == -1 &&
         (stateTime > getCollisionBox(cbId).start)) ||
        stateTime == getCollisionBox(cbId).start) {
...
std::pair<int, int> Character::getPos() { return position; };

StateDef *Character::getCurrentState() { return currentState; };

CollisionBox &Character::getCollisionBox(int cbId) {
  return currentState->collisionBoxes[cbId];
};

int Character::getSoundChannel() { return soundChannel; };

int Character::getAnimScale() { return animScale; };

void Character::setXPos(int x) { position.first = x; };

void Character::setYPos(int y) { position.second = y; };

void Character::setX(int x) { position.first += x; };

void Character::setY(int y) { position.second += y; };

bool Character::hurtState(int state) {
  return (state == specialStateMap[SS_HURT] ||
...
bool Character::airHurtState(int state) {
  return (state == specialStateMap[SS_AIR_HURT] ||
          state == specialStateMap[SS_AIR_HURT_RECOVERY] ||
          state == specialStateMap[SS_BLOWBACK_FALLING] ||
          state == specialStateMap[SS_DEAD_STANDING] ||
          state == specialStateMap[SS_DEAD_KNOCKDOWN] ||
...
bool Character::pushBlockState(int state) {
  return (state == specialStateMap[SS_PUSH_BLOCK] ||
          state == specialStateMap[SS_CROUCH_PUSH_BLOCK] ||
          state == specialStateMap[SS_AIR_PUSH_BLOCK]);
}
...
void Character::setFlag(ObjFlag flag) { flags |= flag; }

void Character::clearFlag(ObjFlag flag) { flags &= (~flag); }

bool Character::getFlag(ObjFlag flag) { return ((flags & flag) == flag); }

void Character::_changeState(int stateNum) { changeState(stateNum); }

void Character::_cancelState(int stateNum) { cancelState(stateNum); }
...
void Character::_velSetY(int ammount) { velocityY = ammount; }
...
void Character::_moveUp(int ammount) { setY(ammount); }

void Character::_moveDown(int ammount) { setY(-ammount); }

void Character::_setControl(int val) { control = val; }
...
void Character::_setGravity(int on) { gravity = on; }

void Character::_setNoGravityCounter(int count) { noGravityCounter += count; }

void Character::_resetAnim() { currentState->resetAnim(); }
...
void Character::_snapToOpponent(int offset) {
  ...
  position.first = opponentFaceRight ? (opponentPos.first + offset)
                                     : (opponentPos.first - offset);
  position.second = opponentPos.second;
  printf("snapping to opponentPos:%d:%d, newPos:%d:%d\n", opponentPos.first,
         opponentPos.second, position.first, position.second);
}
...
int Character::_getStateTime() { return currentState->stateTime; }
...
int Character::_getStateNum() { return currentState->stateNum; }

int Character::_getControl() { return control; }

int Character::_getAirActions() { return hasAirAction; }

void Character::_setHitStun(int operand) { hitstun = operand; }

void Character::_setHitCancel(int val) { currentState->canHitCancel = val; }

void Character::_setWhiffCancel(int val) { currentState->canWhiffCancel = val; }
...
int Character::_getCombo() { return comboCounter; }

int Character::_wasPressed(int input) {
  return virtualController->wasPressedBuffer(
             VirtualController::inputMap[input](inputFaceRight))
             ? 1
             : 0;
}

int Character::_getHitStun() { return hitstun; }

int Character::_getHitCancel() { return currentState->canHitCancel; }

int Character::_getWhiffCancel() { return currentState->canWhiffCancel; }

int Character::_getBlockStun() { return blockstun; }

int Character::_getIsAlive() { return !isDead; }

int Character::_getMeter(int meterIndex) { return meterArray[meterIndex]; }

int Character::_getComebackMeter() { return comeback; }
...
void Character::_setBlockstun(int input) { blockstun = input; };
...
int Character::_getEntityStatus(int entityID) {
  printf("is the entity active? %d, %d\n", entityID,
         entityList[entityID - 1].active);
  return entityList.at(entityID - 1).active;
};

int Character::_getInstall() { return installMode; };

int Character::_getVelY() { return velocityY; };

int Character::_getVelX() { return faceRight ? velocityX : velocityX * -1; };
