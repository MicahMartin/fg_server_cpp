...
FGServer::FGServer() { godot::UtilityFunctions::print("engine constructor"); }

FGServer::~FGServer() { godot::UtilityFunctions::print("engine destructor"); }
...
void FGServer::enter() {
  ...
  std::string p1DefPath = "./data/"
                          "characters/alucard/def.json";
  std::string p2DefPath = "./data/"
                          "characters/alucard/def.json";
...
void FGServer::_ready() {
  ...
  std::string _p1Name(
      this->get_parent()->get_meta("p1Name").stringify().utf8().get_data());
  std::string _p2Name(
      this->get_parent()->get_meta("p2Name").stringify().utf8().get_data());
  std::string _remoteIp(
      this->get_parent()->get_meta("remoteIp").stringify().utf8().get_data());
  remoteIp = _remoteIp;
...
int FGServer::readGodotInputs(int pNum) {
  int inputFrame = 0;
  int inputAxisX = 0;
  int inputAxisY = 0;
...
void FGServer::readGodotTrainingInput() {
...
void FGServer::_physics_process(double delta) {
  // I hate this
  if (godot::Engine::get_singleton()->is_editor_hint()) {
    return;
  }
  ...
  if (netPlayState) {
    ...
    result = ggpo_add_local_input(ggpo, player_handles[netPnum - 1], &input,
                                  sizeof(int));
...
void FGServer::_bind_methods() {
  godot::ClassDB::bind_method(godot::D_METHOD("getModelName", "p_charNum"),
                              &FGServer::getModelName);
  godot::ClassDB::bind_method(godot::D_METHOD("getModelScale", "p_charNum"),
                              &FGServer::getModelScale);
  godot::ClassDB::bind_method(godot::D_METHOD("getGameState"),
                              &FGServer::getGameState);
}
...
void FGServer::step(int inputs[]) {
  ...
  if (!slowMode && !screenFreeze) {
    ...
    if (player1.currentState->checkFlag(SUPER_ATTACK) &&
        (player1.currentState->stateTime ==
         player1.currentState->freezeFrame)) {
      screenFreeze = true;
      screenFreezeLength = player1.currentState->freezeLength;
      player1.activateVisFX(1);
    }

    if (player2.currentState->checkFlag(SUPER_ATTACK) &&
        (player2.currentState->stateTime ==
         player2.currentState->freezeFrame)) {
      screenFreeze = true;
  ...
  if (slowMode) {
    godot::UtilityFunctions::print("Why the fuck are we in slowmode??:",
                                   slowDownCounter);
...
void FGServer::handleRoundStart() {
...
void FGServer::checkEntityHitstop(Character *player) {
...
void FGServer::updateCamera() {
  camera.update(player1.getPos(), player2.getPos());
}

void FGServer::checkThrowCollisions() {
...
void FGServer::checkHitstop(Character *player) {
  if (player->inHitStop && --player->hitStop == 0) {
    player->inHitStop = false;
  }
}

ThrowResult FGServer::checkThrowAgainst(Character *thrower,
                                        Character *throwee) {
  bool canThrow = (thrower->canThrow && thrower->control == true &&
...
void FGServer::handleSameFrameThrowTech(SpecialState techState) {
  player1.control = 0;
  player2.control = 0;
...
void FGServer::checkHitCollisions() {
...
void FGServer::checkEntityHitCollisions() {
  // TODO: func
...
void FGServer::updateVisuals() {
  player1.hitSpark.update();
  player2.hitSpark.update();
}

//** GODOT FUNCTIONS **//
godot::String FGServer::getModelName(int p_charNum) {
  return characters[p_charNum]->modelName.c_str();
}

double FGServer::getModelScale(int p_charNum) {
  return characters[p_charNum]->modelScale;
}

godot::Dictionary FGServer::getGameState() {
...
bool on_event_callback(GGPOEvent *info) {
  ...
  case GGPO_EVENTCODE_CONNECTED_TO_PEER:
    godot::UtilityFunctions::print("GGPO player:", info->u.connected.player,
                                   " is connecting");
  ...
  case GGPO_EVENTCODE_CONNECTION_INTERRUPTED:
    ...
    // timeGetTime(), info->u.connection_interrupted.disconnect_timeout);
    break;
...
#else
    godot::UtilityFunctions::print("you are ", info->u.timesync.frames_ahead,
                                   " frames ahead!");
    // sleep(1000 * info->u.timesync.frames_ahead / 60);
#endif
    break;
...
bool advance_frame_callback(int frame) {
  int inputs[2] = {0};
  int disconnectFlags;
  ggpo_synchronize_input(ggpo, (void *)inputs, sizeof(int) * 2,
                         &disconnectFlags);
  fgServer->step(inputs);
  return true;
}

bool save_state_callback(unsigned char **buffer, int *len, int *checksum,
                         int frame) {
  ...
  *checksum = Util::fletcher32_checksum((short *)*buffer, *len / 2);
  // godot::UtilityFunctions::print("frameCount:", stateObj.frameCount, "
  // checksum:", *checksum);
  return true;
}
...
void free_buffer(void *buffer) { std::free(buffer); }

void FGServer::saveState() {
  ...
  if (!localBuffer) {
    godot::UtilityFunctions::print("error creating buffer of len:!",
                                   localBufferSize);
  } else {
...
void FGServer::pauseState() { shouldUpdate = false; }
void FGServer::unpauseState() { shouldUpdate = true; }

void FGServer::stepState() { stepOnce = true; }

int FGServer::getPort() { return 9998; }

std::string FGServer::getIp() { return "127.0.0.1"; }

void FGServer::ggpoInit() {
