...
//  "@B & !D, N, B",
//  release of down, downforward, anyforward that doesnt include down, lk or
//  release of lk (236K)
//  "~D, DF, @F & !D, LK | ~LK",
//  release of down, downback, any back that doesnt include down, lp or release
//  of lp (214P)
//  "~D, DB, @B & !D, LP | ~LP",
...
void CommandCompiler::init(const char *path) {
  ...
  for (auto &commandStringObj : commandJson["commands"].items()) {
    std::string commandString =
        commandStringObj.value()["command"].get<std::string>();
...
CommandNode CommandCompiler::compileNode() {
  ...
  std::function<bool(Input, bool, int, bool)> funcPointer = std::bind(
      &VirtualController::wasPressedWrapper, controllerPointer, _1, _2, _3, _4);
  CommandNode finalNode;
  CommandFunction &finalFunc = finalNode.function;
  bool strictness = true;
  finalNode.bufferLength = 8;

  while (currentToken->type != CTOKEN_DELIM &&
         currentToken->type != CTOKEN_END) {
    switch (currentToken->type) {
    case CTOKEN_RELEASED: {
      funcPointer = std::bind(&VirtualController::wasReleasedWrapper,
                              controllerPointer, _1, _2, _3, _4);
      godot::UtilityFunctions::print(
          "setting func pointer to wasReleasedWrapper\n");
      currentToken++;
    } break;
    case CTOKEN_HELD: {
      funcPointer = std::bind(&VirtualController::isPressedWrapper,
                              controllerPointer, _1, _2, _3, _4);
      godot::UtilityFunctions::print(
          "setting func pointer to isPressedWrapper\n");
      currentToken++;
    } break;
    case CTOKEN_ANY: {
      strictness = false;
...
CommandFunction CommandCompiler::binaryCommand(CommandFunction currentFunc,
                                               CommandTokenType type) {
  CommandNode nextStatement = compileOneNode();
  CommandFunction returnFunction = [currentFunc, nextStatement,
                                    type](int index, bool faceRight) -> bool {
    if (type == CTOKEN_OR) {
      return (currentFunc(index, faceRight) ||
              nextStatement.function(index, faceRight));
