...
std::map<Input, Input (*)(bool)> VirtualController::inputEnumMap = {
    ...
    {DOWNRIGHT,
     [](bool faceRight) { return faceRight ? DOWNRIGHT : DOWNLEFT; }},
...
void VirtualController::update(uint16_t input) {
  ...
  if (prevStickState != currentStickState) {
    if (prevStickState == 0) {
      ...
      // godot::UtilityFunctions::print(inputToString[Input(prevStickState)], "
      // Release");
    ...
    if (currentStickState == 0) {
      ...
      // godot::UtilityFunctions::print(inputToString[Input(currentStickState)],
      // " Press");
...
bool VirtualController::wasPressedBuffer(Input input, bool strict,
                                         bool pressed) {
  int buffLen = 2;
  // int historySize = inputHistory.size();
  bool found = false;
...
bool VirtualController::checkCommand(int commandIndex, bool faceRight) {
  ...
  std::vector<CommandNode> *command =
      &commandCompiler.commands[commandIndex].command;
    ...
    if (foundPart) {
      ...
      if (i == 0) {
        ...
        if (clears) {
          ...
          // inputHistory.erase(inputHistory.begin() + firstFindOffet,
          // inputHistory.end());
        }
      }
    } else {
...
void VirtualController::toggleRecording() {
  ...
  if (isRecording) {
    isRecording = false;
    godot::UtilityFunctions::print("Recording stopped, size:",
                                   int(recording.size()));
  } else {
    recording.clear();
    isRecording = true;
